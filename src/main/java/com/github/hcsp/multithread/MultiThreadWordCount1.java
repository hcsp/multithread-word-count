...
public class MultiThreadWordCount1 {
    ...
    public static void getFinalResult(Map<String, Integer> finalResult,
                                      List<Future<Map<String, Integer>>> futures) throws ExecutionException, InterruptedException {
    ...
    public static List<Future<Map<String, Integer>>> getFutures(int threadNum, BufferedReader reader) {
        ExecutorService threadPoll = Executors.newFixedThreadPool(threadNum);
        List<Future<Map<String, Integer>>> futures = new ArrayList<>();
        for (int i = 0; i < threadNum; i++) {
            futures.add(threadPoll.submit(new WorkerJob(reader)));
        }
        return futures;
    }
